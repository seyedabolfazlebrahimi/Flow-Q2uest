...
@st.cache_data(ttl=3600)
def load_county_list(api_base: str):  # ✅ NEW
    payload = get_json_with_retry(f"{api_base}/county-list", params=None, tries=6, timeout=180)
    return payload.get("counties", [])


@st.cache_data(ttl=3600)
def load_parameters(api_base: str, huc8s: tuple[str, ...], counties: tuple[str, ...]):  # ✅ counties
...
def load_stations_df(
    ...
    # n_samples optional; keep if exists
...
@st.cache_data(ttl=900)
def load_mean_per_station(api_base: str, parameter: str, huc8s: tuple[str, ...], counties: tuple[str, ...]):  # ✅ NEW
...
# ----------------------------
# 0) Select Spatial Filter (HUC8 | County)
# ----------------------------
...
if mode == "HUC8":
    ...
    if not all_counties:
        st.caption("No counties found (or /county-list not available). Showing all data.")
        selected_counties = []
    else:
        # minimal + simple: just multiselect (no extra search box to avoid changing behavior)
        ...
        if selected_counties:
            st.caption(
                f"Filtering all results to counties: {', '.join(selected_counties[:8])}"
...
with st.expander("Show stations list (map-ready)"):
    if st.button("Load stations"):
        ...
        if df_st.empty:
            st.warning("No stations returned (check filter selection).")
        else:
...
if df_picker.empty:
    st.warning("No stations returned for current filters. Try lowering min_samples, clearing filters, or changing parameters.")
    st.stop()
...
# ----------------------------
# 5) Download by year range  (✅ UNCHANGED)
# ----------------------------
...
if c3.button("Download range CSV from API"):
    params_dl = build_query_params(
        ...
        counties=list(selected_counties_t),   # ✅ NEW
...
# ----------------------------
# 6) Map + time series plot (+ mean)   (✅ UNCHANGED)
# ----------------------------
...
# ----------------------------
# 7) Mean Concentration Map (Blue = Low, Red = High)   (✅ UNCHANGED)
# ----------------------------
